...
<form method="POST" action="{{ route('annonces.update', $annonce->id) }}" class="space-y-4 max-w-md mx-auto mt-4" 
enctype="multipart/form-data">
      ...
      <div class="relative flex items-center">
        <input type="name" name="title" value="{{ old('title', $annonce->title) }}"
      ...
      <div class="relative flex items-center">
        <input type="name" name="price" value="{{ old('price', $annonce->price) }}"
          class="px-4 py-3 pr-10 bg-[#f0f1f2] focus:bg-transparent w-full text-sm border border-gray-200 focus:border-black outline-0 rounded-md transition-all" />
             @error('price')
             <p class="text-red-500">{{ $message }}</p>
             @enderror
      ...
      <div class="relative flex items-center">
        <input type="name" name="location" value="{{ old('location', $annonce->location) }}"
          class="px-4 py-3 pr-10 bg-[#f0f1f2] focus:bg-transparent w-full text-sm border border-gray-200 focus:border-black outline-0 rounded-md transition-all" />
             @error('location')
             <p class="text-red-500">{{ $message }}</p>
             @enderror
 ...
 <div>
      ...
      <div class="relative flex items-center">
        <select type="name" name="condition"
          ...
          <option value="nouveau" {{ old('condition', $annonce->condion)=='nouveau' ? 'selected': '' }}>Nouveau</option>
          <option value="quasi-neuf" {{ old('condition', $annonce->condion)=='quasi-neuf' ? 'selected': '' }}>quasi-neuf</option>
           <option value="utilise"  {{ old('condition', $annonce->condion)=='utilise' ? 'selected': '' }} >utilisé</option>
         </select>
          @error('condition')
             <p class="text-red-500">{{ $message }}</p>
             @enderror
      ...
      <div class="relative flex items-center">
        <select  name="category_id"
          ...
          <option value="{{ $category->id }}" {{ old('category_id', $annonce->category)== $category->id ? 'selected' : '' }}>
            {{ $category->name }}
          </option>
